...
def exists(val):   # return True or False
...
class EGNN(nn.Module):
    ...
    def forward(self, feats, coors, edges = None, mask = None, adj_mat = None):
        ...
        edge_input = torch.cat((feats_i, feats_j, rel_dist), dim = -1)    #eq. (3)

        if exists(edges):
            edge_input = torch.cat((edge_input, edges), dim = -1)   #eq. (3)

        m_ij = self.edge_mlp(edge_input)  #eq. (3)
        ...
        if exists(self.coors_mlp):
            coor_weights = self.coors_mlp(m_ij)    # weights in eq. (4)
            ...
            coors_out = einsum('b i j, b i j c -> b i c', coor_weights, rel_coors) + coors   # eq. (4)
        else:
            coors_out = coors
        ...
        if exists(self.node_mlp):
            ...
            if self.m_pool_method == 'mean':
                if exists(mask):
                    ...
                    m_i = safe_div(m_ij.sum(dim = -2), mask_sum)  # eq. (5)
                else:
                    m_i = m_ij.mean(dim = -2)   # eq. (5)

            elif self.m_pool_method == 'sum':
                m_i = m_ij.sum(dim = -2)  # eq. (5)

            normed_feats = self.node_norm(feats)
            node_mlp_input = torch.cat((normed_feats, m_i), dim = -1)
            node_out = self.node_mlp(node_mlp_input) + feats   # eq. (6)
        else:
            node_out = feats
...
class EGNN_Network(nn.Module):
    def __init__(
        self,
        *,
        depth,  # how many EGNN layers used for our model.
        dim,  # embedding dims
        num_tokens = None,  # how many kinds of atoms, here is None for us
        num_edge_tokens = None,  # how many kinds of edges, here is None for us
        num_positions = None,  # how many positions same number as nodes
        edge_dim = 0,   # edge embbed dims
        ...
        **kwargs
        ...
        #exists(number_adj_degree) and num_adj_degrees < 1  = False, so at least one of it is false
        assert not (exists(num_adj_degrees) and num_adj_degrees < 1), 'make sure adjacent degrees is greater than 1'
        self.num_positions = num_positions

        self.token_emb = nn.Embedding(num_tokens, dim) if exists(num_tokens) else None  # we don't have the atom tokens
    ...
    def forward(
        ...
        feats,
        coors,
        adj_mat = None,  # the adj_mat just stores whether edge exits between two each two nodes (True or False)
        edges = None,   # edges stores the edge information
        mask = None,
        return_coor_changes = False
    ):
        b, device = feats.shape[0], feats.device

        if exists(self.token_emb):   # atom type embbeding
            feats = self.token_emb(feats)

        if exists(self.pos_emb):  # positions embbeding corresponding how many atoms
            n = feats.shape[1]
            ...
            feats += rearrange(pos_emb, 'n d -> () n d')   # a reader-friendly smart element reordering for multi-dimensional tensor, add one dimension by '()'
        ...
        if exists(self.num_adj_degrees):
            ...
            if len(adj_mat.shape) == 2: # extend to all positions
                adj_mat = repeat(adj_mat.clone(), 'i j -> b i j', b = b)  # reordering elements and repeating them in arbitrary combinations
        ...
        global_tokens = None
        if exists(self.global_tokens):   # self.global_tokens is not None
        ...
        for global_attn, egnn in self.layers:
            if exists(global_attn):   # global_attn is not None
