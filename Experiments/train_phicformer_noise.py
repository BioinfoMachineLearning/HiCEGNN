...
parser.add_argument('--mod_name', type=str, default='clof_4ReLU_noise_', help='choice the model you want to use')
parser.add_argument('--epochs', type=int, default=801, help='number of epochs to train (default: 10)')  # default = 100
parser.add_argument('--n_layers', type=int, default=4, help='number of layers for the EGLN')  # test 3 is better than 2, 4 is better than 6
parser.add_argument('--attention', type=int, default=1, help='attention in the EGNN model')
parser.add_argument('--node_dim', type=int, default=512, help='Number of node features at input')  # 16 for lstm embedding, 512 for LINE embedding
    ...
    def fit_model(self):
        ...
        for conversion in conversions:
            ...
            for epoch in range(1, args.epochs):
                ...
                for clean, data, target, info in train_bar:  # clean is the pure image without noise
                ...
                with torch.no_grad():
                    for clean, data, target, info in valid_bar:  # clean is the pure image without noise
                        ...
                        normed = np.loadtxt(data)
                        embeding = np.loadtxt(target)
                        node_attr, edge_ind, mask, edge_attr, _, num_nodes = utils.load_info(normed, embeding,
                                                                                                 device=device)
                        label = np.loadtxt(clean)  # clean normed square matrix as label
                        label = torch.tensor(label, dtype=torch.double).to(device)
        ...
        with open(str(root) + '/Outputs/' + f'finalg_hic{self.mod_name}_{self.res}_{self.cell_Line}_log.txt', 'w') as f:
