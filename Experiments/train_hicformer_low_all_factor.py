...
parser.add_argument('--mod_name', type=str, default='clof_human1_32res_allfactor', help='choice the model you want to use')
...
parser.add_argument('-c', '--conversions', type=str, default='[.1,.1, 2.1]', help='List of conversion constants of the form [lowest, interval, highest] for an set of\
        equally spaced conversion factors, or of the form [conversion] for a single conversion factor.')  # '[.1,.1, 2.1]' or '[.5]'
...
class HiCFormer:
    ...
    def fit_model(self):
            ...
            for epoch in range(1, args.epochs):
                ...
                with torch.no_grad():
                    for data, target, info in valid_bar:  # data is the pure image without noise
                        # if info == 18: # we should exclude the chrom 18 for high resolution
                            # continue
                        data, = data  # used to unpack the only one item in the object such as tuple or list
                        target, = target  # used to unpack the only one item in the object such as tuple or list
                        ...
                        if info == 18:
                            print(f'\n********** the number of chrom {info} node {num_nodes} **************')
                            print(f'\n********** the distance of prediction  {dist_out}')
                            print(f'\n********** the distance of truth {dist_truth}')
 
                            normed_t = normed.cpu().detach().numpy()
                            fig, ax = plt.subplots()  # just one row/colum this will think as one-dimensional
                            show1 = ax.imshow(normed_t, cmap="Reds")
                            ax.set_title("Normalization")
                            fig.colorbar(show1, ax=ax, location='bottom', orientation='horizontal')
                            plt.show()
        ...
        with open(str(root) + '/Outputs/' + f'finalg_hic{self.mod_name}_{self.res}_{self.cell_Line}_{self.cellNo}_log.txt', 'w') as f:
            line1 = f'Optimal conversion factor: {conversions}\n'
            line2 = f'Optimal dSCC: {tempspear}\n'
            line3 = f'Final MSE loss: {tempmse}\n'
